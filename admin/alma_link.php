...
"WHERE link_id = ?";
		...
		header( "Location: http://" . $_SERVER[ "HTTP_HOST" ] . ""
		 . dirname( $_SERVER[ "PHP_SELF" ] ) . "/alma_link.php?catg=" . $catg . "&sub=" . $sub );
		exit;
...
$link_tbl .= "<form method=\"get\" action=\"alma_link.php\">\n";
...
$link_tbl .= "</form>\n";
	...
	if(!$sub){
		$action = "alma_link.php";
	}else{
		$action = "alma_link.php?catg=" . $catg . "&sub=" . $sub;
	}
