<?PHP
// *******************************************************************
// ENGLISH管理画面記事更新 PHP Encording UTF-8
// *******************************************************************
	...
	if( $_POST[ "contents_submit" ] ){
		...
		if($_POST[ "contents_alias" ] != "" && $_POST[ "contents_alias" ] != $bf_alias && $_POST[ "contents_alias" ] != "redirect" ){
		...
		if(!$err_msg){
			...
			header( "Location: http://" . $_SERVER[ "HTTP_HOST" ] . ""
			 . dirname( $_SERVER[ "PHP_SELF" ] ) . "/eng_content_list.php?catg=" . $_POST[ "contents_catg" ] );
			exit;
		}
	}

	// 画像削除
	if( isset($_GET['image_del']) && isset($_GET[ "image_name" ]) && isset($_GET[ "id" ]))
	{

		$image_name=urldecode($_GET[ "image_name" ]);
		// ファイル削除
		$image_path = "../eng/images/contents/" . $id . "/" . $image_name;
		...
		//自身ヘリダイレクト
		header( "Location: http://" . $_SERVER[ "HTTP_HOST" ] . ""
			. dirname( $_SERVER[ "PHP_SELF" ] ) . "/eng_content_modify.php?id=" . $id . "&catg=" . $catg );
		exit;
...
' WHERE '
	...
	// 画像確認
	$target_dir = "../eng/images/contents/" . $id . "/";
	...
	if($up_flg){
		...
		while ($file = readdir($dh)) {
			...
			$img_tmp = BASE_URL . "eng/images/contents/" . $id . "/" . $file;
			$img_file .= "<a href=\"" . $img_tmp . "\" onclick=\"return hs.expand(this)\" class=\"highslide\">" . $img_tmp . "</a>&nbsp;\n";
			$img_file .= "<a href=\"#\" id=\"copy-desc" . $j . "\">URLコピー</a>\n";
			$img_file .= "<span style=\"display:none\" id=\"desc" . $j . "\">" . $img_tmp . "</span>";
			$img_file .= "&nbsp;<a href=\"eng_content_modify.php?id=" . $id . "&catg=" . $catg . "&image_del=1&image_name=" . urlencode($file) . "\">削除</a></span>";
			$img_file .= "<br />\n";
			$j++;
		}
		closedir($dh);
	...
	$confirm2 = "記事編集";
	$action = "eng_content_modify.php?catg=" . $catg . "&id=" . $id;
	$contents_submit = "ページ編集";
	$sk_url = BASE_URL . "eng/?p=" . $id;
	$bs_url = BASE_URL . "eng/";
	if($contents_alias){
		$mr_url = BASE_URL . "eng/" . $contents_alias;
	}
?>