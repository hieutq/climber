...
"WHERE link_id = ?";
		...
		header( "Location: http://" . $_SERVER[ "HTTP_HOST" ] . ""
		 . dirname( $_SERVER[ "PHP_SELF" ] ) . "/eng_link.php?catg=" . $catg . "&sub=" . $sub );
		exit;
...
$link_tbl .= "<form method=\"get\" action=\"eng_link.php\">\n";
...
$link_tbl .= "</form>\n";
	...
	if(!$sub){
		...
		$action = "eng_link.php?catg=" . $catg . "&sub=" . $sub;
	}
