<?PHP
// *******************************************************************
// ICP管理画面記事新規作成　PHP　Encording UTF-8
// *******************************************************************
...
' ORDER BY '
	.'cts_id DESC';
		...
		$cts_plus = $cts_id + 1;
		header( "Location: http://" . $_SERVER[ "HTTP_HOST" ] . ""
		 . dirname( $_SERVER[ "PHP_SELF" ] ) . "/icp_content_regist.php?id=" . $cts_plus );
		exit;
	...
	if( $_POST[ "contents_submit" ] != NULL ){
		...
		if(!$err_msg){
			...
			// 設定
			$htaccess = "../icp/.htaccess"; // ファイル格納場所
	...
	$confirm2 = "記事新規登録";
	$action = "icp_content_regist.php?id=" . $id;
