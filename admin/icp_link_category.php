...
"WHERE lctgr_id = ?";
			...
			header( "Location: http://" . $_SERVER[ "HTTP_HOST" ] . ""
			 . dirname( $_SERVER[ "PHP_SELF" ] ) . "/icp_link_category.php" );
			exit;
...
"WHERE sctgr_id = ? ";
			...
			header( "Location: http://" . $_SERVER[ "HTTP_HOST" ] . ""
			 . dirname( $_SERVER[ "PHP_SELF" ] ) . "/icp_link_category.php?sub=" . $sub );
			exit;
...
$catg_tbl .= "<form method=\"get\" action=\"icp_link_category.php\">\n";
...
$catg_tbl .= "</form>\n";
	...
	if(!$sub){
		$action = "icp_link_category.php";
	}else{
		$action = "icp_link_category.php?sub=" . $sub;
	}
