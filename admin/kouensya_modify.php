...
// ----------------------------------------
	...
	if( $_POST[ "OK_1" ] != "" ){
		...
		$old_datas = kouensya_Read_By_ID( $_SESSION[ 'JILM_KOUEN_ID' ] );
		$_SESSION[ $s_name ][ 'old_kouen_userid' ] = $old_datas[ 'kouen_userid' ];
		...
		$err_msg = kouensya_Error_Check($_SESSION[ $s_name ], 'admin_modify');
		...
		if( $err_msg == "" ){
			...
			// 一覧にリダイレクト
			header( "Location: http://" . $_SERVER[ "HTTP_HOST" ] . ""
			 . dirname( $_SERVER[ "PHP_SELF" ] )	. "/kouensya_modify_confirm.php" );
			exit;
		}
...
// ----------------------------------------
	...
	if ( $_SESSION[ $s_name ] == NULL ) {
		...
		// データ読出
		$_SESSION[ $s_name ] = kouensya_Read_By_ID( $_SESSION[ 'JILM_KOUEN_ID' ] );

		$pay_date = $_SESSION[ $s_name ][ 'pay_date' ];
		$_SESSION[ $s_name ][ 'pay_date_y' ] = substr($pay_date,0,4);
		$_SESSION[ $s_name ][ 'pay_date_m' ] = substr($pay_date,4,2);
		$_SESSION[ $s_name ][ 'pay_date_d' ] = substr($pay_date,6,2);
	...
	// デフォルト値設定
	// --------------------------------------------------------
	if ( $_SESSION[ $s_name ][ 'pay_date_y' ] == '' ) { $_SESSION[ $s_name ][ 'pay_date_y' ] = date( 'Y' ); }
	if ( $_SESSION[ $s_name ][ 'pay_date_m' ] == '' ) { $_SESSION[ $s_name ][ 'pay_date_m' ] = date( 'm' ); }
	if ( $_SESSION[ $s_name ][ 'pay_date_d' ] == '' ) { $_SESSION[ $s_name ][ 'pay_date_d' ] = date( 'd' ); }
