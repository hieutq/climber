...
// ----------------------------------------
	...
	if( $_POST[ "OK_1" ] != "" ){
		...
		// エラーチェック
		$err_msg = sankasya_Data_Error_Check( $_SESSION[ $s_name ], 'admin_regist' );
		...
		if( $err_msg == "" ){
			...
			// 一覧にリダイレクト
			header( "Location: http://" . $_SERVER[ "HTTP_HOST" ] . ""
			 . dirname( $_SERVER[ "PHP_SELF" ] )	. "/sankasya_regist_confirm.php" );
			exit;
...
function sankasya_Data_Session_Set($s_name) {
	...
	if ( isset( $_SESSION[ $s_name ][ 'sanka_menu_array' ]) ) {
		$_SESSION[ $s_name ][ 'sanka_menu' ] = implode( ",", $_SESSION[ $s_name ][ 'sanka_menu_array' ] );
	} else {
		$_SESSION[ $s_name ][ 'sanka_menu' ] = array();
	}
