<?PHP
	...
	if( $_POST[ "OK_1" ] != "" ){
		...
		// 入力データを変数に変換
		$regist_body  = StripSlashes( $_POST[ "regist_whn_body" ] );
		$regist_datey = StripSlashes( $_POST[ "regist_whn_datey" ] );
		$regist_datem = StripSlashes( $_POST[ "regist_whn_datem" ] );
		$regist_dated = StripSlashes( $_POST[ "regist_whn_dated" ] );
	...
	$sql .= " WHERE status < 2";
	$sql .= " ORDER BY -info_date, -update_dt";
	$res  = cn_query($sql, $cnn);
		...
		if ($max_cnt > 0){
			...
			for ($i=1; $i<=$max_page; $i++) {
				if ($dsp_page != 1 && $i == $dsp_page - 1) {
					$page_link1 = "[ <a href='whats_new.php?dsp_page=$i'>前の" . $dsp_cnt . "件</a> ]";
				}
				if ($dsp_page != $max_page && $i == $dsp_page + 1) {
					$page_link2 = "[ <a href='whats_new.php?dsp_page=$i'>次の" . $dsp_cnt . "件</a> ]";
				}
			}
			...
			for ($i=0; $i<$max_cnt; $i++) {
				if ( $i >= $stt_cnt && $i <= $end_cnt ) {
					...
					$info_id   = cn_contract($res, $i, "info_id");
					$body_tmp  = cn_contract($res, $i, "info_body");
					$status    = cn_contract($res, $i, "status");
					$tmp_date  = cn_contract($res, $i, "info_date");
					...
					$tbl .= "<input type='hidden' name='info_id' value='";
					$tbl .= $info_id . "'>\n<td>" . $info_id . "</td>\n";
					$tbl .= "<td>" . $info_body . "</td>\n";
					$tbl .= "<td>" . $info_date . "</td>\n";
					$tbl .= "<td><a href='whats_new_modify.php?id=";
					$tbl .= $info_id . "&mode=bang'>修正</a>&nbsp;";
