<?php
// *******************************************************************
// ALMA新着情報　PHP　Encording UTF-8
// *******************************************************************

	session_start();
	include_once( "./../../include/az_constant.php" );
	include_once( "./../../include/alma_admin.php" );
	include_once( "./../../admin/alma_info.php" );
...
$(document).ready(function(){
	...
	$("#confirm1").click(function(){
		var button = $(this);
		if (confirm || jConfirm("新着情報を登録します\nよろしいですか？", "新着情報登録",function(r){
			if (r == true) {
				confirm = true;
				button.click();
			}
...
<div id="h_halt">
	<label>トピックス文章</label><br />
	<input name="info_body" type="text" class="text" size="48" value="" />
</div>
<div id="normal">
	<label>リンク先URL</label><br />
	<input name="info_url" type="text" class="text" size="48" value="" />
</div>
<div id="h_bottom">
	<label>登録表示日</label>
	<select name="info_datey" class="dropdown">
<?=$datey?>
	</select>年&nbsp;
	<select name="info_datem" class="dropdown">
<?=$datem?>
	</select>月&nbsp;
	<select name="info_dated" class="dropdown">
<?=$dated?>
	</select>日&nbsp;
...
<option value='alma_info.php?status=0'>表示中のアイテムのみ</option>
<option value='alma_info.php?status=1'<?=$status_op?>>全表示</option>
</select>
</div>
</form>
