...
//***********************************************************************
	...
	include_once( "./../../src/part_conv_kouen_entry.php" );
...
// 講演申込み期間かどうかチェックする
// 大会idが最新の一件しかチェックしないので、2番目が講演期間中でもここではじいている
$flg = convention_Date_Check( 'kouen' );

// 遅延申込みチェック
if ( ! $flg ) {

	// 一連の作業中許可するために、セッションをまずチェック
	if ( ! $_SESSION[ $s_name ][ 'chien_status' ] ) {
		if ( $_GET[ 'key' ] != '' ) {
			$chien_key = stripslashes( $_GET[ 'key' ] );
			$_GET[ 'key' ] = NULL;
			$chien_flg = convention_Chien_ID_Check($dsp['convention_id'], $chien_key); // 日付制限も確認
			if ( $chien_flg ) {
				convention_Chien_Access_Update($dsp['convention_id'], $chien_key); // アクセス済にする
				$flg = TRUE; // 申込み可能にする
				$_SESSION[ $s_name ][ 'chien_status' ] = 1; // セッションで遅延申込み許可を発行
			}
		}
	} else {
		$flg = TRUE; // セッションがあれば許可
	}
...
<div class="main_box">
<h3>講演申込入力フォーム</h3>
<p style="font-size: 12px;">〔注1〕講演申込にあたって、講演形式を、口頭発表/ポスター発表/どちらでもよい/テーマセッション（6テーマ）から選択下さい。口頭発表またはどちらでもよいを選択した場合は、続いて講演分類を選択下さい。<br>
〔注2〕今回より講演分類と選択方法を変更しました。大分類（対象材料）および5つの小分類（現象、用途、検出・解析方法、目的、材料形状）からそれぞれ該当するものを選択下さい。従来と異なる切り口でのプログラム編成を考えています。
</p>
...
<td>
	<input type="text" name="ko_title" size="40" value="{$dsp['kouen_title']}" /><br />
...
<td>
	<input type="text" name="ko_title_eng" size="40" value="{$dsp['kouen_title_eng']}" /><br />
...
<td>
	...
	ローマ字：<input type="text" name="k_name_eng[]" size="30" value="{$dsp['member_name_eng'][0]}" />&nbsp;半角(スペースは入れない）　　例：T.Keikinzoku<br />
...
<td>{$link_display_none[0]}
	...
	ローマ字：<input type="text" name="k_name_eng[]" size="30" value="{$dsp['member_name_eng'][1]}" />&nbsp;半角(スペースは入れない）　　例：T.Keikinzoku<br />
...
<td>{$link_display_none[1]}
	...
	ローマ字：<input type="text" name="k_name_eng[]" size="30" value="{$dsp['member_name_eng'][2]}" />&nbsp;半角(スペースは入れない）　　例：T.Keikinzoku<br />
...
<td>{$link_display_none[2]}
	...
	ローマ字：<input type="text" name="k_name_eng[]" size="30" value="{$dsp['member_name_eng'][3]}" />&nbsp;半角(スペースは入れない）　　例：T.Keikinzoku<br />
...
<td>{$link_display_none[3]}
	...
	ローマ字：<input type="text" name="k_name_eng[]" size="30" value="{$dsp['member_name_eng'][4]}" />&nbsp;半角(スペースは入れない）　　例：T.Keikinzoku<br />
...
<td>{$link_display_none[4]}
	...
	ローマ字：<input type="text" name="k_name_eng[]" size="30" value="{$dsp['member_name_eng'][5]}" />&nbsp;半角(スペースは入れない）　　例：T.Keikinzoku<br />
...
<td>{$link_display_none[5]}
	...
	ローマ字：<input type="text" name="k_name_eng[]" size="30" value="{$dsp['member_name_eng'][6]}" />&nbsp;半角(スペースは入れない）　　例：T.Keikinzoku<br />
...
<td>{$link_display_none[6]}
	...
	ローマ字：<input type="text" name="k_name_eng[]" size="30" value="{$dsp['member_name_eng'][7]}" />&nbsp;半角(スペースは入れない）　　例：T.Keikinzoku<br />
...
<td>
<input type="text" name="userid" size="30" value="{$dsp[ 'kouen_userid']}" />&nbsp;非会員の場合は0を入力してください
</td>
</tr>
...
<td>
<input type="text" name="name" size="30" value="{$dsp[ 'kouen_name']}" />
</td>
</tr>
...
<td><input type="text" name="mail" size="30" style="ime-mode:disabled;" value="{$dsp[ 'kouen_mail']}" /></td>
</tr>
<tr>
...
<tr>
<th><span class="fc_red">※</span>講演発表・登録料<br />支払方法</th>
<td>$input_radio_2</td>
</tr>
...
<input type="hidden" name="info01" value="{$dsp[ 'kouen_info01']}" />
<input type="hidden" name="tel" value="{$dsp[ 'kouen_tel']}" />
<input type="hidden" name="fax" value="{$dsp[ 'kouen_fax']}" />
<input type="hidden" name="zip1" value="{$dsp[ 'kouen_zip1']}" />
<input type="hidden" name="zip2" value="{$dsp[ 'kouen_zip2']}" />
<input type="hidden" name="address" value="{$dsp[ 'kouen_address']}" />
