...
//	$convention_id  = convention_Data_Maxid_Get();
	...
	// 初期化
	if( $referer ) {
		if ( strpos( $referer, 'conv_kouen_edit' ) ) {
...
//		$_SESSION[ $s_name ][ 'pay_status' ]  = 1; // 未入金にする（入金=status 2)
		...
		$err_msg = kouensya_Error_Check($_SESSION[ $s_name ], 'pc_modify');
		...
		if( $err_msg == "" ){
			...
			header( "Location: http://" . $_SERVER[ "HTTP_HOST" ] . ""
			 . dirname( $_SERVER[ "PHP_SELF" ] )	. "/conv_kouen_edit_confirm.php" );
			exit;
...
// ----------------------------------------
	...
	if ( $_SESSION[ $s_name ] == NULL ) {
		...
		$search_array[ 'kouen_id' ] = intval( $_SESSION[ 'JILM_KOUEN_ID' ] );
		$_SESSION[ $s_name ] = kouensya_Read_Custom( $search_array );
	}
