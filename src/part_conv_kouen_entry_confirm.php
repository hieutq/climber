...
// ------------------------------------
	if ( $_POST[ 'CONV_KOUEN_SEND' ] != '' ) {
		...
		// 書き込み処理
		// --------------------------------
		$_SESSION[ $s_name ][ 'kouen_uid' ] = kouensya_Data_Insert( $_SESSION[ $s_name ] );
		...
		kouensya_Data_Send_Mail( $_SESSION[ $s_name ], $mail_options, 'pc_regist' );
		...
		// 一覧にリダイレクト
		header( "Location: http://" . $_SERVER[ "HTTP_HOST" ] . ""
		 . dirname( $_SERVER[ "PHP_SELF" ] )	. "/conv_kouen_entry_fin.php" );
		exit;
...
// ******************************************************************
	$action = 'conv_kouen_entry_confirm.php';
	$dsp = $_SESSION[ $s_name ];
