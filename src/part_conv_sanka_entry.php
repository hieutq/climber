...
// ----------------------------------------
	...
	$pay_menu_text_type     = $CONFIG_PAY_MENU_TYPE_LONG;
...
//		$_SESSION[ $s_name ][ 'member_userid' ]  = stripslashes( $_POST[ 'userid' ] );
	...
	if( $err_msg == "" ){
		...
		$_SESSION[ $s_name ][ 'sanka_menu_text' ] = sankasya_Data_Get_Menu_Text(
													$sanka_array   = $_SESSION[ $s_name ][ 'sanka_menu_array' ],
													$pay_menu_type = $pay_menu_text_type,
													$separator     = ','
											);

		// 参加項目明細入り
		$money_list = sankasya_Data_Make_PriceText_List(
													$member_kubun = $_SESSION[ $s_name ][ 'member_kubun01' ],
													$conv_id = $_SESSION[ $s_name ][ 'convention_id' ],
													$price_name_list = 	$CONFIG_PAY_MENU_TYPE_LONG,
													$mode='each'
											);
		$pay_money_list = array();
		$money_count = count($money_list);
		for($i=1;$i<=$money_count;$i++){
			if( in_array( $i, $_SESSION[ $s_name ][ 'sanka_menu_array' ] ) ) {
				 $pay_money_list[$i] = $money_list[$i];
			}
		}
		$_SESSION[ $s_name ][ 'pay_money_list' ] = implode("\n", $pay_money_list);
...
// ----------------------------------------
	...
	// 参加項目配列生成
	if( !$_SESSION[ $s_name ][ 'member_kubun01' ]) {
		$mem_kubun = 99;
	} else {
		$mem_kubun = $_SESSION[ $s_name ][ 'member_kubun01' ];
	}
	$pay_menu_type = sankasya_Data_Make_PriceText_List(
							$member_kubun = $mem_kubun,
